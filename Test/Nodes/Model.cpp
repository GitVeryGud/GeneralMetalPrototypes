...
void Model::render( MTL::RenderCommandEncoder* pCommandEncoder, Uniforms* uniforms ) {
    ...
    pCommandEncoder->setVertexBuffer(textureBuffer, 0, 1);
    ...
    for (int i = 0; i < scene->mNumMeshes; i++) {
        ...
        for (int j = 0; j < currentMesh->mNumVertices; j++) {
            ...
            if (currentMesh->HasTextureCoords(0)) {
                textureVertices[j + vertexTrack].x = currentMesh->mTextureCoords[0][j].x;
                textureVertices[j + vertexTrack].y = currentMesh->mTextureCoords[0][j].y;
            }
        }
